...
function playVideo() {
  ...
  setMainImage();
  changeElementDisplay(game, 'none');
  changeElementDisplay(video, 'block');
  video.play();
  video.addEventListener('ended', function() {
    changeElementDisplay(game, 'block');
    changeElementDisplay(video, 'none');
    if (index === gameArray.length) {
      setMainImage();
      removeMainListener();
      endGame();
...
function checkAnswer(event) {
  ...
  if (event.target.name === gameArray[index - 1]) {
    ...
    rightAnswer.addEventListener('ended', function() {
      changeElementDisplay(quiz, 'none');
      addQuizListeners();
      playVideo();
    })
  } else {
...
function updateGame() {
  ...
  if (clickCount === 1 || clickCount % 2 !== 0) {
    ...
    updateQuizImages();
    changeElementDisplay(game, 'none');
    changeElementDisplay(quiz, 'flex');
    removeQuizListeners();
...
function setupGame() {
  ...
  setMainImage();
  changeElementDisplay(game, 'block');
  changeElementDisplay(options, 'none');
...
function changeOptionChoices() {
  ...
  for (let i = 0; i < gameOrderButtons.length; i++) {
    changeElementDisplay(gameOrderButtons[i], 'none');
    changeElementDisplay(imageTypes[i], 'inline-block');
  }
}
